...
public function getAllRefundRequests(): array
    {
        try {
            $refunds = $this->refundRequestService->getPendingRequests();
            $result = [];
            foreach ($refunds as $refund) {
                $user = $this->userRepository->find($refund->getRequesterId());
                $result[] = [
                    'id' => $refund->getId(),
                    'requester_id' => $refund->getRequesterId(),
                    'user_name' => $user ? $user->getUsername() : 'Unknown',
                    'user_email' => $user ? $user->getEmail() : 'Unknown',
            ...
            $this->logger->error('Failed to get all refund requests', ['error' => $e->getMessage()]);
            return [];
        }
    }
    ...
    {
        return $this->refundRequestService->countPendingRequests();
    }
